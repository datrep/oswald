...
#   -Host <addr>       bind address written to .env (default 0.0.0.0)
...
function Write-Fail([string]$msg) { Write-Host "  [FAIL] $msg" -ForegroundColor $Error }
...
$webHost    = if ($Host -eq '0.0.0.0') { 'localhost' } else { $Host }
...
try {
    ...
    $certHost = if ($Host -eq '0.0.0.0') { 'localhost' } else { $Host }
...
Write-Host "  Dashboard : http://$Host`:$AppPort   (HTTPS :$HttpsPort)" -ForegroundColor $Info
Write-Host "  Fileserver: https://$Host`:8090" -ForegroundColor $Info
