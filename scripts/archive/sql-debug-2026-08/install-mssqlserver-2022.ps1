# install-mssqlserver-2022.ps1 — install SQL 2022 DEFAULT instance (MSSQLSERVER)
# from the full 2022 media at C:\SQL2022\Express_ENU, then restart-survival test.
# Self-elevates; logs to temp\sql-mssqlserver-2022.log
$root    = (Resolve-Path (Join-Path $PSScriptRoot '..')).Path
$logDir  = Join-Path $root 'temp'
New-Item -ItemType Directory -Path $logDir -Force | Out-Null
$logFile = Join-Path $logDir 'sql-mssqlserver-2022.log'
...
Log "==== SQL 2022 DEFAULT INSTANCE INSTALL $(Get-Date) ===="
...
if ((SvcStatus 'MSSQLSERVER') -eq 'Running') {
    ...
    if ((SvcStatus 'MSSQLSERVER') -eq 'Running') { Log "RESULT: DEFAULT INSTANCE SURVIVES RESTART - SUCCESS" } else { Log "RESULT: DEFAULT INSTANCE FAILS ON RESTART" }
} else {
